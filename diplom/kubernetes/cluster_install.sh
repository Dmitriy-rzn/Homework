...
cd kubespray
ansible-playbook --key-file ~/.ssh/id_rsa -i inventory/mycluster/hosts.ini --become cluster.yml

cd ../terraform
MASTER_1_PRIVATE_IP=$(terraform output -json group_masters_private_ips | jq -j ".[0]")
MASTER_1_PUBLIC_IP=$(terraform output -json group_masters_public_ips | jq -j ".[0]")
